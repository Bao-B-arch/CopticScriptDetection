...
from common.types import NDArrayBool, NDArrayFloat, NDArrayInt, NDArrayStr, NDArrayUInt
...
@pytest.mark.parametrize(
        ...
        [
            (
                {"alpha": np.array([[[1, 1, 3], [1, 1, 3], [2, 2, 2]], [[0, 0, 3], [0, 0, 1], [3, 1, 2]]])}, 
                2, 4, 3,
                (
                    np.array([f"mean_{i}" for i in range(4)] + [f"var_{i}" for i in range(4)]),
                    np.array(["alpha", "alpha"], dtype="<U10"),
                    np.array([[1.0, 2.0, 1.5, 2.0, 0.0, 1.0, 0.25, 0.5], [0.0, 1.0, 1.0, 1.0, 0.0, 1.5, 1.5, 0.5]], dtype=np.float64),
